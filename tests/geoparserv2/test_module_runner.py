...
from geoparser.db.models import (
    RecognitionModule as RecognitionModuleModel,
    ResolutionModule as ResolutionModuleModel
)
...
def test_initialize_recognition_module_new(test_db, mock_recognition_module):
    ...
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(RecognitionModuleRepository, "get_by_name_and_config", return_value=None) as mock_get:
            new_module = RecognitionModuleModel(id=uuid.uuid4(), name="mock_recognition", config={"param": "value"})
            with patch.object(RecognitionModuleRepository, "create", return_value=new_module) as mock_create:
                # Call the method
                module_id = runner._initialize_recognition_module(mock_recognition_module)
                
                # Verify calls
                mock_get.assert_called_once_with(test_db, mock_recognition_module.name, mock_recognition_module.config)
...
def test_initialize_recognition_module_existing(test_db, mock_recognition_module):
    ...
    existing_module = RecognitionModuleModel(id=uuid.uuid4(), name="mock_recognition", config={"param": "value"})
    
    # Mock database calls
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(RecognitionModuleRepository, "get_by_name_and_config", return_value=existing_module) as mock_get:
            # Call the method
            module_id = runner._initialize_recognition_module(mock_recognition_module)
...
def test_initialize_resolution_module_new(test_db, mock_resolution_module):
    ...
    # Mock database calls for module creation
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(ResolutionModuleRepository, "get_by_name_and_config", return_value=None) as mock_get:
            new_module = ResolutionModuleModel(id=uuid.uuid4(), name="mock_resolution", config={"param": "value"})
            with patch.object(ResolutionModuleRepository, "create", return_value=new_module) as mock_create:
                # Call the method
                module_id = runner._initialize_resolution_module(mock_resolution_module)
                
                # Verify calls
                mock_get.assert_called_once_with(test_db, mock_resolution_module.name, mock_resolution_module.config)
                mock_create.assert_called_once()
                assert module_id == new_module.id
...
def test_initialize_resolution_module_existing(test_db, mock_resolution_module):
    ...
    # Create existing module in database
    existing_module = ResolutionModuleModel(id=uuid.uuid4(), name="mock_resolution", config={"param": "value"})
    
    # Mock database calls
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(ResolutionModuleRepository, "get_by_name_and_config", return_value=existing_module) as mock_get:
            # Call the method
            module_id = runner._initialize_resolution_module(mock_resolution_module)
...
def test_run_module_recognition(test_db, test_project, mock_recognition_module):
    ...
    with patch.object(runner, "_initialize_recognition_module") as mock_init:
        with patch.object(runner, "_execute_recognition_module") as mock_exec:
            ...
            mock_exec.assert_called_once_with(mock_recognition_module, module_id, test_project.id)
...
def test_run_module_resolution(test_db, test_project, mock_resolution_module):
    ...
    with patch.object(runner, "_initialize_resolution_module") as mock_init:
        with patch.object(runner, "_execute_resolution_module") as mock_exec:
            ...
            mock_exec.assert_called_once_with(mock_resolution_module, module_id, test_project.id)
...
def test_execute_recognition_module(test_db, test_project, mock_recognition_module, test_document):
    ...
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(runner, "_get_unprocessed_documents") as mock_get_docs:
            with patch.object(runner, "_process_toponym_predictions") as mock_process:
                ...
                runner._execute_recognition_module(mock_recognition_module, module_id, test_project.id)
                
                # Verify method calls
                mock_get_docs.assert_called_once_with(test_db, module_id, test_project.id)
                mock_recognition_module.predict_toponyms.assert_called_once_with([test_document.text])
...
def test_execute_recognition_module_no_documents(test_db, test_project, mock_recognition_module):
...
def test_execute_resolution_module(test_db, test_project, mock_resolution_module, test_toponym, test_document):
    ...
    with patch("geoparser.geoparserv2.module_runner.get_db", return_value=iter([test_db])):
        with patch.object(runner, "_get_unprocessed_toponyms") as mock_get_tops:
            with patch.object(runner, "_process_location_predictions") as mock_process:
                ...
                runner._execute_resolution_module(mock_resolution_module, module_id, test_project.id)
...
def test_execute_resolution_module_no_toponyms(test_db, test_project, mock_resolution_module):
...
def test_process_toponym_predictions(test_db, mock_recognition_module):
    ...
    with patch.object(runner, "_create_toponym_record") as mock_create:
        with patch.object(runner, "_mark_document_processed") as mock_mark:
            ...
            runner._process_toponym_predictions(test_db, document_ids, predicted_toponyms, module_id)
...
def test_create_toponym_record(test_db, test_document):
    ...
    with patch.object(ToponymRepository, "create") as mock_create_toponym:
        with patch.object(RecognitionObjectRepository, "create") as mock_create_object:
            ...
            result = runner._create_toponym_record(test_db, test_document.id, 10, 15, module_id)
...
def test_process_location_predictions(test_db, mock_resolution_module):
    ...
    with patch.object(runner, "_create_location_record") as mock_create:
        with patch.object(runner, "_mark_toponym_processed") as mock_mark:
            ...
            runner._process_location_predictions(test_db, toponym_ids, predicted_locations, module_id)
...
def test_create_location_record(test_db, test_toponym):
    ...
    with patch.object(LocationRepository, "create") as mock_create_location:
        with patch.object(ResolutionObjectRepository, "create") as mock_create_object:
            ...
            result = runner._create_location_record(test_db, test_toponym.id, "loc1", 0.8, module_id)
