...
def test_recognition_service_initialization():
    ...
    with patch.object(
        RecognitionService, "_load_recognizer_record", return_value=recognizer_id
    ):
        service = RecognitionService(mock_recognizer)
        assert service.recognizer == mock_recognizer
        assert service.recognizer_id == recognizer_id


def test_load_recognizer_record_existing(test_db):
    """Test _load_recognizer_record with an existing recognizer."""
    mock_recognizer = MagicMock()
    mock_recognizer.name = "test_recognizer"
    ...
    with patch("geoparser.services.recognition.Session") as mock_session:
        ...
        with patch.object(
            ...
            service = RecognitionService(mock_recognizer)
            assert service.recognizer_id == existing_recognizer_id


def test_load_recognizer_record_new(test_db):
    """Test _load_recognizer_record with a new recognizer."""
    mock_recognizer = MagicMock()
    mock_recognizer.name = "test_recognizer"
    mock_recognizer.config = {"param": "value"}
    ...
    with patch("geoparser.services.recognition.Session") as mock_session:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                service = RecognitionService(mock_recognizer)
                assert service.recognizer_id == new_recognizer_id
