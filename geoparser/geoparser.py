...
class Geoparser:
    def __init__(self, spacy_model='en_core_web_trf', transformer_model='dguzh/geo-all-distilroberta-v1', gazetteer='geonames'):
    ...
    def setup_gazetteer(self, gazetteer):
        ...
        if gazetteer_name in GAZETTEERS:
            ...
            raise ValueError(f"Invalid gazetteer name. Available gazetteers: {available}")

    def setup_spacy(self, spacy_model):
        if not spacy.util.is_package(spacy_model):
            raise OSError(f"spaCy model '{spacy_model}' not found. Run the following command to install it:\npython -m spacy download {spacy_model}")

        spacy.prefer_gpu()
        
        nlp = spacy.load(spacy_model)
        nlp.make_doc = lambda text: GeoDoc(self, nlp.vocab,
    ...
    def parse(self, texts: List[str], batch_size=8, country_filter: List[str] = None, feature_filter: List[str] = None):
        ...
        docs = list(tqdm(self.nlp.pipe(texts, batch_size=batch_size), total=len(texts), desc="Batches"))
    ...
    def resolve(self, docs: List[GeoDoc], batch_size=8):
        ...
        if candidate_ids:
            ...
            candidate_descriptions = [self.gazetteer.get_location_description(location) for location in self.gazetteer.query_location_info(candidate_ids)]
            candidate_embeddings = self.transformer.encode(candidate_descriptions, batch_size=batch_size, show_progress_bar=True, convert_to_tensor=True)
            candidate_embeddings_lookup = dict(zip(candidate_ids, candidate_embeddings))

            toponym_contexts = [toponym.context.text for doc in docs for toponym in doc.toponyms]
            toponym_embeddings = self.transformer.encode(toponym_contexts, batch_size=batch_size, show_progress_bar=True, convert_to_tensor=True)

            toponym_index = 0
            for doc in docs:
                for toponym in doc.toponyms:
                    candidate_ids = toponym.candidates
                    candidate_embeddings = [candidate_embeddings_lookup[candidate_id] for candidate_id in candidate_ids]
