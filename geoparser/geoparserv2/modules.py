...
class BaseModule(ABC):
    ...
    def run(self, session_id: UUID) -> None:
        ...
        try:
            ...
            logging.info(f"Module '{self.name}' completed successfully on session {session.name} ({session.id})")
        except Exception as e:
            # Rollback in case of error
            db.rollback()
            logging.error(f"Error executing module '{self.name}' on session {session_id}: {str(e)}")
            raise
        finally:
...
class RecognitionModule(BaseModule):
    ...
    def _document_has_been_processed(self, db: DBSession, document: Document) -> bool:
        ...
        for toponym in toponyms:
            ...
            if any(recognition.module_id == self.module.id for recognition in recognitions):
                return True
    ...
    def _execute(self, db: DBSession, session: Session) -> None:
        ...
        for document in documents:
            ...
            if self._document_has_been_processed(db, document):
                logging.info(f"Document {document.id} already processed by module '{self.name}', skipping.")
                continue
            ...
            logging.info(f"Document {document.id} processed by module '{self.name}', {len(created_toponyms)} toponyms found.")
...
class ResolutionModule(BaseModule):
    ...
    def _get_toponyms(self, db: DBSession, session: Session) -> t.List[t.Tuple[Document, Toponym]]:
        """
    ...
    def _toponym_has_been_processed(self, db: DBSession, toponym: Toponym) -> bool:
        ...
        for location in locations:
            ...
            if any(resolution.module_id == self.module.id for resolution in resolutions):
                return True
    ...
    def _execute(self, db: DBSession, session: Session) -> None:
        ...
        for document, toponym in doc_toponym_pairs:
            ...
            if self._toponym_has_been_processed(db, toponym):
                logging.info(f"Toponym {toponym.id} already processed by module '{self.name}', skipping.")
                continue
            ...
            predicted_locations = self.predict_locations(document.text, toponym_text, toponym.start, toponym.end)
            ...
            logging.info(f"Toponym {toponym.id} ('{toponym_text}') processed by module '{self.name}', {len(created_locations)} locations found.")
