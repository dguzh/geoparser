...
class GeoparserAnnotator(Geoparser):
    ...
    def setup_routes(self):
        ...
        def upload():
            ...
            return render_template('upload.html', gazetteers=gazetteers, spacy_models=spacy_models, cache_exists=cache_exists)
        ...
        def annotate():
            ...
            for doc_item in self.documents:
                ...
                # Identify toponyms
                toponyms = [ent for ent in doc_obj_item.ents if ent.label_ in ["GPE", "LOC", "FAC", "ANNOT"]]
                doc_item['total_toponyms'] = len(toponyms)

                # Count annotated toponyms
                annotations = doc_item.get('annotations', [])
                annotated_toponym_set = set((ann['start'], ann['end']) for ann in annotations)
                annotated_count = sum(1 for ent in toponyms if (ent.start_char, ent.end_char) in annotated_toponym_set)
                doc_item['annotated_toponyms'] = annotated_count

            pre_annotated_text = self.get_pre_annotated_text(doc_obj, doc.get('annotations', []))
        ...
        def get_candidates():
            ...
            filter_attributes = [
                col.name for col in location_columns
                if col.type == 'TEXT' and
                col.name != location_identifier and
                not col.name.endswith(location_identifier)
            ]

            # Find existing annotation for this toponym
            existing_annotation = next(
                (ann for ann in annotations if ann['start'] == start and ann['end'] == end),
                None
            )
            existing_loc_id = existing_annotation['loc_id'] if existing_annotation else None
        ...
        def save_annotation():
            ...
            for idx, existing_annotation in enumerate(annotations):
                if existing_annotation['start'] == annotation['start'] and existing_annotation['end'] == annotation['end']:
            ...
            doc_obj = doc['doc_obj']
            toponyms = [ent for ent in doc_obj.ents if ent.label_ in ["GPE", "LOC", "FAC", "ANNOT"]]
