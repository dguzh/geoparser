...
class SQLGenerator:
    ...
    def _optimize_spatial_join_condition(self, condition: str) -> str:
        ...
        # Pattern to match spatial function calls
        # Matches: FUNCTION(geometry1, table2.geometry2)
        for func in spatial_functions:
            # Special handling for ST_Centroid which is nested
            if "ST_Centroid" in condition:
                # Pattern for ST_Within(ST_Centroid(table1.geom), table2.geom)
                pattern = rf"(ST_\w+)\s*\(\s*ST_Centroid\s*\(\s*([^)]+)\)\s*,\s*(\w+)\.(\w+)\s*\)"
                ...
                if match:
                    ...
                    spatial_index_condition = (
                        ...
                        f"WHERE f_table_name = '{geometry2_table}' AND search_frame = {geometry1_with_centroid})"
                    )
