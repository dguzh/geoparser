...
class Gazetteer(ABC):
    ...
    def get_location_description(self, location):
        return self.format_location_description(location, self.location_description_template)
    ...
    def format_location_description(location, template):
        if location:
            ...
            formatted_text = re.sub(r"(?P<pre>[^\s<>]*?)<(?P<key>\w+)>(?P<post>[^\s<>]*)", substitute_keys, template)
