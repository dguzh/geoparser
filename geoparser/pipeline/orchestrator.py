...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        if recognition_module:
            logger.info(f"Pipeline initialized with recognition module: {recognition_module.name}")
            
        if resolution_module:
            logger.info(f"Pipeline initialized with resolution module: {resolution_module.name}")
    ...
    def run_recognition(self, session_id: UUID) -> None:
        ...
        if not self._should_run_recognition(session_id):
            logger.info(f"Skipping recognition module: {self.recognition_module.name} on session: {session_id}")
            return

        logger.info(f"Running recognition module: {self.recognition_module.name} on session: {session_id}")
        self.recognition_module.run(session_id)
        logger.info(f"Completed recognition module: {self.recognition_module.name} on session: {session_id}")
    ...
    def run_resolution(self, session_id: UUID) -> None:
        ...
        if self.recognition_module is None:
            logger.warning("No recognition module registered - needed to identify source toponyms")
            return
        
        # Check if we need to run this module
        if not self._should_run_resolution(session_id):
            logger.info(f"Skipping resolution module: {self.resolution_module.name} on session: {session_id}")
            return
            
        logger.info(f"Running resolution module: {self.resolution_module.name} on session: {session_id}")
        self.resolution_module.run(session_id)
        logger.info(f"Completed resolution module: {self.resolution_module.name} on session: {session_id}")
