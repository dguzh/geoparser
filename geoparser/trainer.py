...
class GeoparserTrainer(Geoparser):
    ...
    def evaluate(self, eval_docs):
        ...
        for doc in tqdm(eval_docs):
            for toponym in doc.toponyms:
                ...
                if gold_id is None:
                    ...
                    predicted_location = self.gazetteer.query_location_info([predicted_id])[0]

                    if gold_location is None:
                        continue
    
                    distance = haversine((gold_location['latitude'], gold_location['longitude']),
        ...
        # Calculate AUC
        adjusted_distances = np.array(distances) + 1  # Avoid zero distance for log scale
